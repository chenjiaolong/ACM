...
/********************************************************************************************
射线法判断点q与多边形 polygon 的位置关系，要求 polygon 为简单多边形，顶点逆时针排列
   如果点在多边形内：   返回0
   如果点在多边形边上： 返回1
   如果点在多边形外： 返回2
********************************************************************************************/
...
bool InsideConvexPolygon(int vcount, POINT polygon[], POINT q) // 可用于三角形！
{
    ...
    p.x = 0;
    p.y = 0;
    for(int i = 0; i < vcount; i++) // 寻找一个肯定在多边形 polygon 内的点 p：多边形顶点平均值
    {
        p.x += polygon[i].x;
        p.y += polygon[i].y;
    }
...
void Graham_scan(POINT PointSet[], POINT ch[], int n, int &len)
{
    ...
    PointSet[k] = tmp; // 现在 PointSet 中y坐标最小的点在 PointSet[0]
    for (int i = 1; i < n - 1; i++) /* 对顶点按照相对PointSet[0]的极角从小到大进行排序，极角相同的按照距离PointSet[0]从近到远进行排序 */
    {
        k = i;
        for (int j = i + 1; j < n; j++)
...
bool LinesegInsidePolygon(int vcount, POINT polygon[], LINESEG l)
{
    ...
    for(int i = 0; i < top - 1; i++) /* 冒泡排序，x 坐标小的排在前面；x 坐标相同者，y 坐标小的排在前面 */
    {
...
void AddRegion ( double x1, double y1, double x2, double y2, double &xtr, double &ytr, double &wtr, double &xtl, double &ytl, double &wtl )
{
    ...
    if ( x2 > x1 )
    {
        ...
        AddPosPart ((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3, (x2 - x1) * (y2 - y1) / 2, xtr, ytr, wtr);// triangle 全局变量变化处
    }
    else
    {
        AddNegPart ((x2 + x1) / 2, y1 / 2, (x2 - x1) * y1, xtl, ytl, wtl);// rectangle 全局变量变化处
        AddNegPart ((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3, (x2 - x1) * (y2 - y1) / 2, xtl, ytl, wtl);// triangle  全局变量变化处
    }
}
POINT cg_simple(int vcount, POINT polygon[])
{
    double xtr, ytr, wtr, xtl, ytl, wtl;
    //注意： 如果把 xtr,ytr,wtr,xtl,ytl,wtl改成全局变量后这里要删去
    POINT p1, p2, tp;
    xtr = ytr = wtr = 0.0;
    xtl = ytl = wtl = 0.0;
    for(int i = 0; i < vcount; i++)
...
POINT a_point_insidepoly(int vcount, POINT polygon[])
{
    ...
    int index = 0;
    v = polygon[0];
    for(int i = 1; i < vcount; i++) //寻找一个凸顶点
    {
        if(polygon[i].y < v.y)
        {
            v = polygon[i];
    ...
    int in1 = index;
    bool bin = false;
    for(int i = 0; i < vcount; i++) //寻找在三角形 avb 内且离顶点 v 最近的顶点 q
    {
        if(i == index) continue;
...
First take the convex hull of the points. 	the resulting convex
...
L = {V0, V1, ,Vn-1} is any n-vertex polylin
Set start = 0;
Set k = 0;
Set W0 = V0;
...
球坐标公式：
直角坐标为 P(x, y, z) 时，
对应的球坐标是(rsinφcosθ, rsinφsinθ, rcosφ),
其中 φ是向量OP与Z轴的夹角，范围[0，π]；
是OP在XOY面上的投影到 X 轴的旋角，范围[0，2π]
直线的一般方程转化成向量方程：ax+by+c=0
x-x0     y-y0
 ------ = ------- // (x0,y0)为直线上一点，m,n为向量
m        n
转换关系：
...
int main(int argc, char** argv) {
	...
	POINT ob1[3] = {(1, 1), (1, 2), (2, 3)};//不管这里赋任何值，输出的最近距离都是5，而且点都是（0，2.0197e-307）
	POINT ob2(3, 4);
	POINT ob3;
	double d = ptopointset(number, ob1, ob2, ob3);
	cout << d << endl;
